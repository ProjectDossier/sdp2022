...
def agg_preds(x, weighting_scheme: str = "free"):

    x["agg_preds"] = 0
    fields = list(x["mode"].values)
    rel_fields = ["description", "title"]
    if weighting_scheme == "free":
        if "description" in fields and len(x[~x["mode"].isin(rel_fields)]) > 0:
            # title + description + add fields
            x.loc[x["mode"] == "description", "agg_preds"] = np.multiply(
                x[x["mode"] == "description"].predictions, 0.25
            )
            x.loc[x["mode"] == "title", "agg_preds"] = np.multiply(
                x[x["mode"] == "title"].predictions, 0.25
            )
            weight = 0.5 / len(x[~x["mode"].isin(rel_fields)])
            x.loc[~x["mode"].isin(rel_fields), "agg_preds"] = np.multiply(
                x[~x["mode"].isin(rel_fields)].predictions, weight
            )
            ...
            x.loc[x["mode"] == "title", "agg_preds"] = np.multiply(
                x[x["mode"] == "title"].predictions, 0.5
            )

        elif "description" in fields:
            # title + abstract
            x.loc[x["mode"] == "description", "agg_preds"] = np.multiply(
                x[x["mode"] == "description"].predictions, 0.5
            )
            x.loc[x["mode"] == "title", "agg_preds"] = np.multiply(
                x[x["mode"] == "title"].predictions, 0.5
            )
            ...
            # only title
            x.loc[x["mode"] == "title", "agg_preds"] = x[
                x["mode"] == "title"
            ].predictions

    elif weighting_scheme == "uniform":
        weight = 1 / len(x)
        x["agg_preds"] = np.multiply(x.predictions, weight)
    ...
    def __call__(
        ...
        if self.pred_samples is not None:
            ...
            if len(pred_samples.core_id.unique()) != len(pred_samples):
                ...
                pred_samples.drop_duplicates(
                    subset="core_id", inplace=False, ignore_index=True
                )
            ...
            pred_classes = np.argmax(
                np.vstack(tuple(pred_samples[pred_field])), axis=1
            ).tolist()
            ...
            pred_samples = pd.merge(
                pred_samples, pd.DataFrame.from_dict(preds), on="core_id"
            )
            pred_samples.reset_index(drop=True, inplace=True)

            # map classes to actual labels
            pred_samples["theme"] = pred_samples.replace({"theme": self.map_classes})[
                "theme"
            ]
        ...
        if labels is not None and -1 not in labels and len(labels) > 0:
            ...
            if output_path is not None and self.write_csv:
                ...
                with open(
                    csv_path, mode="a" if out_file_exists else "w", encoding="utf-8"
                ) as f:
                    writer = csv.writer(f)
